 ...
 * @package thrift.transport
 */
class TSocketPool extends SmppTransport\TSocket {
        ...
        for ($i = 0; $i < $numServers; ++$i) {
            ...
            if ($lastFailtime > 0) {
                ...
                if ($elapsed > $this->retryInterval_) {
                    ...
                    if ($this->debug_) {
                        call_user_func($this->debugHandler_, 'TSocketPool: retryInterval ' .
                                '(' . $this->retryInterval_ . ') ' .
            ...
            if (($lastFailtime === 0) ||
                ...
                for ($attempt = 0; $attempt < $this->numRetries_; $attempt++) {
                    try {
                        ...
                        return;
                    } catch (SmppTransport\TException $tx) {
                        // Connection failed
                    }
                }
                ...
                if ($consecfails >= $this->maxConsecutiveFailures_) {
                    if ($this->debug_) {
                        call_user_func($this->debugHandler_, 'TSocketPool: marking ' . $host . ':' . $port .
