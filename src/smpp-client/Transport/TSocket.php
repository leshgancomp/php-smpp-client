 ...
 */
class TSocket extends SmppTransport\TTransport {
    ...
    public function open() {
        if ($this->isOpen()) {
            throw new SmppTransport\TTransportException('Socket already connected', SmppTransport\TTransportException::ALREADY_OPEN);
        }

        if (empty($this->host_)) {
            throw new SmppTransport\TTransportException('Cannot open null host', SmppTransport\TTransportException::NOT_OPEN);
        }

        if ($this->port_ <= 0) {
            throw new SmppTransport\TTransportException('Cannot open without port', SmppTransport\TTransportException::NOT_OPEN);
        }
        ...
        if ($this->handle_ === FALSE) {
            ...
            throw new SmppTransport\TException($error);
        }
    }
    ...
    public function read($len) {
        ...
        if ($readable > 0) {
            ...
            if ($data === false) {
                throw new SmppTransport\TTransportException('TSocket: Could not read ' . $len . ' bytes from ' .
                $this->host_ . ':' . $this->port_);
            } elseif ($data == '' && feof($this->handle_)) {
                throw new SmppTransport\TTransportException('TSocket read 0 bytes');
            }

            return $data;
        } else if ($readable === 0) {
            throw new SmppTransport\TTransportException('TSocket: timed out reading ' . $len . ' bytes from ' .
            $this->host_ . ':' . $this->port_);
        } else {
            throw new SmppTransport\TTransportException('TSocket: Could not read ' . $len . ' bytes from ' .
            $this->host_ . ':' . $this->port_);
        ...
        while (strlen($buf) > 0) {
            ...
            if ($writable > 0) {
                ...
                if ($written === -1 || $written === false) {
                    throw new SmppTransport\TTransportException('TSocket: Could not write ' . strlen($buf) . ' bytes ' .
                    $this->host_ . ':' . $this->port_);
                }
                // determine how much of the buffer is left to write
                $buf = substr($buf, $written);
            } else if ($writable === 0) {
                throw new SmppTransport\TTransportException('TSocket: timed out writing ' . strlen($buf) . ' bytes from ' .
                $this->host_ . ':' . $this->port_);
            } else {
                throw new SmppTransport\TTransportException('TSocket: Could not write ' . strlen($buf) . ' bytes ' .
                $this->host_ . ':' . $this->port_);
