...
use Exploit\Smpp\Protocol as SmppProtocol;
...
class SmppClient {
     ...
     */
    public function __construct(SmppTransport\TTransport $transport, $debugHandler = null) {
        // Internal parameters
    ...
    public function bindReceiver($login, $pass) {
        ...
        $response = $this->_bind($login, $pass, SmppProtocol\SMPP::BIND_RECEIVER);
    ...
    public function bindTransmitter($login, $pass) {
        ...
        $response = $this->_bind($login, $pass, SmppProtocol\SMPP::BIND_TRANSMITTER);
    ...
    public function close() {
        ...
        $response = $this->sendCommand(SmppProtocol\SMPP::UNBIND, "");
    ...
    public function readSMS() {
        $command_id = SmppProtocol\SMPP::DELIVER_SM;
        // Check the queue
...
//check for enquire link command
            if ($pdu->id == SmppProtocol\SMPP::ENQUIRE_LINK) {
                $response = new SmppProtocol\SmppPdu(SmppProtocol\SMPP::ENQUIRE_LINK_RESP, SmppProtocol\SMPP::ESME_ROK, $pdu->sequence, "\x00");
     ...
     */
    public function sendSMS(SmppProtocol\SmppAddress $from, SmppProtocol\SmppAddress $to, $message, $tags = null, $dataCoding = SmppProtocol\SMPP::DATA_CODING_DEFAULT, $priority = 0x00, $scheduleDeliveryTime = null, $validityPeriod = null) {
        $msg_length = strlen($message);

        if ($msg_length > 160 && $dataCoding != SmppProtocol\SMPP::DATA_CODING_UCS2 && $dataCoding != SmppProtocol\SMPP::DATA_CODING_DEFAULT)
            return false;

        switch ($dataCoding) {
            case SmppProtocol\SMPP::DATA_CODING_UCS2:
                ...
                break;
            case SmppProtocol\SMPP::DATA_CODING_DEFAULT:
        ...
        if ($doCsms) {
            if (self::$sms_use_msg_payload_for_csms) {
                $payload = new SmppProtocol\SmppTag(SmppProtocol\SmppTag::MESSAGE_PAYLOAD, $message, $msg_length);
                ...
                $sar_msg_ref_num = new SmppProtocol\SmppTag(SmppProtocol\SmppTag::SAR_MSG_REF_NUM, $csmsReference, 2, 'n');
                $sar_total_segments = new SmppProtocol\SmppTag(SmppProtocol\SmppTag::SAR_TOTAL_SEGMENTS, count($parts), 1, 'c');
                $seqnum = 1;
                foreach ($parts as $part) {
                    $sartags = array($sar_msg_ref_num, $sar_total_segments, new SmppProtocol\SmppTag(SmppProtocol\SmppTag::SAR_SEGMENT_SEQNUM, $seqnum, 1, 'c'));
     ...
     * @return string message id
     */
    protected function submit_sm(SmppProtocol\SmppAddress $source, SmppProtocol\SmppAddress $destination, $short_message = null, $tags = null, $dataCoding = SmppProtocol\SMPP::DATA_CODING_DEFAULT, $priority = 0x00, $scheduleDeliveryTime = null, $validityPeriod = null) {
        // Construct PDU with mandatory fields
        $pdu = pack('a1cca' . (strlen($source->value) + 1) . 'cca' . (strlen($destination->value) + 1) . 'ccc' . ($scheduleDeliveryTime ? 'a16x' : 'a1') . ($validityPeriod ? 'a16x' : 'a1') . 'ccccca' . (strlen($short_message) + (self::$sms_null_terminate_octetstrings ? 1 : 0)), self::$sms_service_type, $source->ton, $source->npi, $source->value, $destination->ton, $destination->npi, $destination->value, self::$sms_esm_class, self::$sms_protocol_id, $priority, $scheduleDeliveryTime, $validityPeriod, self::$sms_registered_delivery_flag, self::$sms_replace_if_present_flag, $dataCoding, self::$sms_sm_default_msg_id, strlen($short_message), //sm_length
        ...
        $response = $this->sendCommand(SmppProtocol\SMPP::SUBMIT_SM, $pdu);
     ...
     * @param integer $dataCoding (optional)
     */
    protected function splitMessageString($message, $split, $dataCoding = SmppProtocol\SMPP::DATA_CODING_DEFAULT) {
        switch ($dataCoding) {
            case SmppProtocol\SMPP::DATA_CODING_DEFAULT:
                $msg_length = strlen($message);
                ...
                $parts[] = $part;
                return $parts;
            case SmppProtocol\SMPP::DATA_CODING_UCS2: // UCS2-BE can just use str_split since we send 132 octets per message, which gives a fine split using UCS2
            default:
                return str_split($message, $split);
        }
    }
    ...
    protected function _bind($login, $pass, $command_id) {
        ...
        $pduBody = pack(
                ...
                'CCCa' . (strlen(self::$address_range) + 1), $login, $pass, self::$system_type, self::$interface_version, self::$addr_ton, self::$addr_npi, self::$address_range
        );

        $response = $this->sendCommand($command_id, $pduBody);
        if ($response->status != SmppProtocol\SMPP::ESME_ROK)
            throw new SmppProtocol\SmppException(SmppProtocol\SMPP::getStatusMessage($response->status), $response->status);
     ...
     */
    protected function parseSMS(SmppProtocol\SmppPdu $pdu) {
        // Check command id
        if ($pdu->id != SmppProtocol\SMPP::DELIVER_SM)
        ...
        $source = new SmppProtocol\SmppAddress($source_addr, $source_addr_ton, $source_addr_npi);

        $dest_addr_ton = next($ar);
        $dest_addr_npi = next($ar);
        $destination_addr = $this->getString($ar, 21);
        $destination = new SmppProtocol\SmppAddress($destination_addr, $dest_addr_ton, $dest_addr_npi);
        ...
        if (($esmClass & SmppProtocol\SMPP::ESM_DELIVER_SMSC_RECEIPT) != 0) {
            $sms = new SmppProtocol\SmppDeliveryReceipt($pdu->id, $pdu->status, $pdu->sequence, $pdu->body, $service_type, $source, $destination, $esmClass, $protocolId, $priorityFlag, $registeredDelivery, $dataCoding, $message, $tags);
            $sms->parseDeliveryReceipt();
        } else {
            $sms = new SmppProtocol\SmppSms($pdu->id, $pdu->status, $pdu->sequence, $pdu->body, $service_type, $source, $destination, $esmClass, $protocolId, $priorityFlag, $registeredDelivery, $dataCoding, $message, $tags);
        }
        ...
        $response = new SmppProtocol\SmppPdu(SmppProtocol\SMPP::DELIVER_SM_RESP, SmppProtocol\SMPP::ESME_ROK, $pdu->sequence, "\x00");
        $this->sendPDU($response);
        return $sms;
    }
    ...
    public function enquireLink() {
        $response = $this->sendCommand(SmppProtocol\SMPP::ENQUIRE_LINK, null);
        return $response;
    }
    ...
    protected function sendCommand($id, $pduBody) {
        if (!$this->transport->isOpen())
            return false;
        $pdu = new SmppProtocol\SmppPdu($id, 0, $this->sequence_number, $pduBody);
        $this->sendPDU($pdu);
        $response = $this->readPDU_resp($this->sequence_number, $pdu->id);
        if ($response === false)
            throw new SmppProtocol\SmppException('Failed to read reply to command: 0x' . dechex($id));
     ...
     * @param SmppPdu $pdu
     */
    protected function sendPDU(SmppProtocol\SmppPdu $pdu) {
    ...
    protected function readPDU_resp($seq_number, $command_id) {
        ...
        for ($i = 0; $i < $ql; $i++) {
            $pdu = $this->pdu_queue[$i];
            if (
                    ($pdu->sequence == $seq_number && ($pdu->id == $command_id || $pdu->id == SmppProtocol\SMPP::GENERIC_NACK)) ||
                    ($pdu->sequence == null && $pdu->id == SmppProtocol\SMPP::GENERIC_NACK)
            ) {
        ...
        do {
            $pdu = $this->readPDU();
            if ($pdu) {
                if ($pdu->sequence == $seq_number && ($pdu->id == $command_id || $pdu->id == SmppProtocol\SMPP::GENERIC_NACK))
                    return $pdu;
                if ($pdu->sequence == null && $pdu->id == SmppProtocol\SMPP::GENERIC_NACK)
                    return $pdu;
    ...
    protected function readPDU() {
        ...
        if ($this->debug) {
            ...
            call_user_func($this->debugHandler, " command status  : 0x" . dechex($command_status) . " " . SmppProtocol\SMPP::getStatusMessage($command_status));
    ...
    protected function parseTag(&$ar) {
        ...
        $tag = new SmppProtocol\SmppTag($id, $value, $length);
        if ($this->debug) {
